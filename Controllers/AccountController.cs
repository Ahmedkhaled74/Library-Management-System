        ...
        public async Task<IActionResult> Borrow(int id)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (applicationUser != null)
                {
                    ...
                    List<Checkout> checkoutActive = new List<Checkout>();
                    foreach (var item in checkoutdb)
                    {
                        if(item .ActiveFlag==true)
                            checkoutActive.Add(item);
                    }
                    var check= checkoutActive.MinBy(c => c.DueDate);
                    if(check != null)
                    {
                        ViewBag.availableDate = DateOnly.FromDateTime(check.DueDate);
                    }
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (applicationUser != null)
                {
                    ...
                    foreach (var item in checkoutdb)
                    {
                        ...
                        if (item.UserId == applicationUser.Id && item.BookId == id && item.ActiveFlag==true)
                        {
                            TimeSpan lateDate = DateTime.Now.Date - item.CheckoutDate;
                           // ViewBag.lateTax = Math.Max(lateDate, 0) * 5;
                            item.Penalty= Math.Max(lateDate.Days, 0) * 5;
